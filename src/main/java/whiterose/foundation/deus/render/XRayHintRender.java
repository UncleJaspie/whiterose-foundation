...
import whiterose.foundation.deus.modules.XRaySelect;
...
public class XRayHintRender implements IItemRenderer {
    ...
    @Override public void renderItem(ItemRenderType type, ItemStack item, Object ... data) {
        GL11.glPushMatrix();
        if (block.hidden) {
            DeusAPI.render.GUI.drawRect(0, 14, 8, 16, Colors.RED);
        }
        if (block.tracer) {
            DeusAPI.render.GUI.drawRect(8, 14, 16, 16, Colors.SKY);
        }
        DeusAPI.render.GUI.drawBorderRect(1, 1, 15, 15, 1, Colors.TRANSPARENT, block.rgb);
        RenderHelper.enableGUIStandardItemLighting();
        RenderItem.getInstance().renderItemIntoGUI(DeusAPI.utils.mc().fontRenderer, DeusAPI.utils.mc().getTextureManager(), item, 0, 0);
        GL11.glPopMatrix();
    }
