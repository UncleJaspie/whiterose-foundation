...
import whiterose.foundation.deus.api.exceptions.ConfigTypeException;
import whiterose.foundation.deus.api.module.CheatModule;
import whiterose.foundation.deus.handlers.ModuleHandler;

public class Config {
    
    private static final File configDir = new File(System.getProperty("user.home") + "/" + DeusAPI.mod_id);
    ...
    public static void load() {
        try {
            ...
            DeusAPI.logger.error("Config reset due to error reading from file: " + e.getMessage());
            save();
        }
    }
                ...
                for (Field field : clazz.getDeclaredFields()) {
                    if (field.isAnnotationPresent(Cfg.class)) {
                        ...
                        try {
                            ...
                            DeusAPI.logger.error("error while saving the ModuleConfig field: " + module + " -> " + field.getName() + " [" + e.getMessage() + "]");
                            e.printStackTrace();
                        }
                    }
                }
            ...
            if (module != null) {
                FLDS.forEach((FLD, DATA) -> {
                    ...
                    try {
                        ...
                        DeusAPI.logger.error("error while reading the ModuleConfig field: " + module + " -> " + cField + " [" + e.getMessage() + "]");
                        e.printStackTrace();
                    }
                });
            }
