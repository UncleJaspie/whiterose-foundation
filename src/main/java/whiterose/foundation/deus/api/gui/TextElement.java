...
public abstract class TextElement extends GuiElement {
    ...
    public void setText(String text) {
        setWidth(render.GUI.deusFont().textWidth(text) + indentX);
        setHeight(render.GUI.deusFont().fontHeight() + indentY);
        this.text = text;
    }
    ...
    protected int getTextX(String in) {
        switch (aligment) {
        case CENTER:
            return getX() + (getWidth() - render.GUI.deusFont().textWidth(in)) / 2;
        case RIGHT:
            return getX() + (getWidth() - render.GUI.deusFont().textWidth(in));
        default:
            return getX();
        }
    }
